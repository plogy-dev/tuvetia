...
- Path: `{user_id}/{patient_id}.{ext}` (el id del usuario que sube la foto, **no** `clinic_id`). Policies en `storage.objects` (insert/update/delete) exigen `(storage.foldername(name))[1] = auth.uid()::text`.
- **Por qué no `clinic_id` vía `private.my_clinic_id()`:** la primera versión de las policies usaba `(storage.foldername(name))[1] = private.my_clinic_id()::text`, igual que en las tablas normales. En Postgres funcionaba bien en simulaciones manuales, pero en producción real las subidas fallaban con `"new row violates row-level security policy for table \"objects\""` (confirmado en los logs de `postgres`, service `storage`) — la sesión propia del servicio de Storage no resuelve `auth.uid()` de forma consistente para una función `SECURITY DEFINER` cross-schema dentro de su policy, aunque esa misma función funciona perfecto en policies de PostgREST (`patients_select`, etc.). Se cambió a comparar contra `auth.uid()` directo (sin función intermedia, sin subquery a `profiles`) porque es el patrón oficial de Supabase para RLS en Storage y evita esa dependencia. Si se vuelve a tocar esto, **no asumas que una simulación con `set local request.jwt.claims` en `execute_sql` es representativa** — dio resultados inconsistentes entre sí durante el debugging; solo el log real de `storage` (`get_logs service=storage` + grep en `postgres` por `"row-level security policy for table \"objects\""`) fue confiable.
- Flujo de subida (`create-patient-drawer.tsx`): se crea el paciente primero vía RPC, y solo si hay foto se sube a Storage y se hace un `update` de `patients.photo_url` con la URL pública. Si la subida falla, el paciente igual queda creado (se avisa con un toast que ahora incluye el mensaje real del error, y se loguea en consola con `console.error`).
