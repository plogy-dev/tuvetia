...
- `GET /health`.
- **`athos_actions` (tabla nueva, NO implementada por este servicio).** Existe en producción, vacía, para que Athos **proponga acciones** (`tool_name`, `payload`, `risk: auto|approval`, `source: chat|inbox|auto`) que un humano aprueba (`status: proposed→approved→executed`). Su RLS tiene **sólo SELECT**, así que el escritor previsto es un backend con `service_role` (nosotros) y el front sólo lee. Antes de implementarla hay que acordar el contrato con Pipe/Felipe: hoy tampoco existe la ruta de aprobación del lado del front. Ver `docs/ESTADO-PLATAFORMA-2026-07-28.md`.
...
- ⚠️ **NO es el retrato del esquema de producción.** Auditado el 2026-07-28: el principal tiene **56 tablas** y el repo declara **28**. Las otras 31 (facturación, inventario, compras, comunicaciones de cobranza y `athos_actions`) se aplicaron directo a la base y **no existen como SQL ni como código en el repo**. Antes de asumir qué hay en producción, leé `docs/ESTADO-PLATAFORMA-2026-07-28.md`. El principal además lleva su propio historial de migraciones con versiones tipo `20260727073858`; las nuestras se aplican a mano y no quedan registradas ahí.
