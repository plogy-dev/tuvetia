...
- `GET /health`.
- `POST /athos/retrieve` — retrieval determinístico para el **agente de Next** (su tool `search_clinical_evidence`). Body `{ question, species?, patient_id?, clinic_id }`. Corre la cascada (A→B + tiers) SIN el LLM de redacción y devuelve los mejores chunks con extracto.
