...
**2026-07-23 (noche) — 🎨 Visual estilo Artifacts (Phantom + Copiloto), env de Vercel alineadas y disciplina de alineación en tiempo real**
> Se llevó la estética de los mockups (Artifacts) a las pantallas reales, se blindó la alineación de despliegue y se cerró el seam de `consultations.status`. Todo en vivo en `master`; build verde (`tsc` + `next build`).
- **Visual (nuestras pantallas):** `consultas/[id]` (Phantom) tomó el diseño del Artifact *mockup-nota-athos*: cabecera de paciente con chips, sección **"Alertas de la consulta"** con paneles expandibles — **crítico rojo** para el gate de alergia (bloqueante, tag "En [paciente]") y **warn** para condiciones (diabetes/ERC, "Ver afectaciones · En [paciente]") — y SOAP con badges **S/O/A/P**. El Copiloto (`asistente`) sumó el aviso **"Hilo con memoria"** (ya tenía citas `[n]` enlazadas + `SourceCard`). Tokens shadcn `neutral`; **sin tocar la lógica**. *Nota: los mockups (`docs/mockup-*.html`, Artifacts) eran prototipos sueltos que nunca estuvieron conectados — por eso "no se reflejaban"; ahora sí.*
- **Env de Vercel alineadas:** `NEXT_PUBLIC_ATHOS_URL` estaba como **dos entradas separadas** (prod/preview) → se alineó la de **preview** a la URL canónica del backend para que los **previews de compañeros peguen al mismo Athos** y no rompan. Regla: `NEXT_PUBLIC_*` iguales en `production` y `preview` (ver §🔒).
- **Seam `consultations.status` CERRADO (nuestro):** el front avanza la consulta a **`review`** al generar la sugerencia y a **`completed`** al aprobar la nota (antes quedaba atascada en `generating_note`).
- **Disciplina de alineación en tiempo real (probada):** el equipo pusheó muy seguido — entraron **PR #4** (Calendario + fix E5 "captura en historia"), **PR #5** (historia de paciente maestro-detalle) y **PR #6** (calendar hardening + purga de audio). Cada uno se **reconcilió ANTES de tocar nada** (`git fetch` + fast-forward/rebase), **sin conflictos y sin pisar trabajo ajeno** (se construyó sobre su versión). Un push a `master` fue rechazado por *non-fast-forward* → me puse al día y rebaseé. **Regla:** antes de cada cambio, `git fetch` y partir de `master`; revisar constantemente.
- **EN VIVO ahora:** front `https://tuvetia.vercel.app` (= último `master`) + backend `https://athos-service-production.up.railway.app`. Copiloto + Phantom + captura + transcripción + gates + Calendario + historia de paciente = **todo en el mismo link, `READY`**.

**2026-07-24 — 🎨 Fidelidad total a los mockups (Copiloto + Nota) + consulta general + citas [n] en el SOAP**
> Las pantallas de Athos ya se ven y se usan como los Artifacts, con todas las funcionalidades reales. Todo en vivo en `master`; build + tests en verde.
