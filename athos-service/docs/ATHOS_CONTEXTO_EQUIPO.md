...
- **Pendiente:** afinar el prompt para que `allergy_transcript_flag` sea confiable también al abstenerse; revisar `citations` del Chat (hoy top-8 determinístico, sin filtrar por lo que el modelo usó); ampliar corpus (conejo/exóticos); comparar **Opus 4.8 vs Sonnet** contra este golden (`--model`).
